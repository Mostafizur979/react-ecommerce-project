...
function ProductsCard({product,index}){
    ...
    return(
        <div key={index} className='w-full shadow-2xl p-[10px] rounded-[5px]'>
            ...
            <button className='text-[14px] sm:text-[15px] md:text-[17px] bg-[#EFEFEF] p-[5px] rounded-[5px]'
            onClick={()=>{
                ...
                getData('https://fakestoreapi.com/products/' + (index+1),(index+1));

            }}
            >Add to Cart</button>
            <button className='text-[14px] sm:text-[15px] md:text-[17px] bg-[#B34BF8] p-[5px] rounded-[5px] text-white'>Buy Now</button>
            </div>
            <ToastContainer />
        </div>
    )
